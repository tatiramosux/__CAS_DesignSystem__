...
export default function TextInputPage() {
  ...
  return (
    <>
      ...
      <section className="section-block"><p className="section-index">COMPONENT STRUCTURE</p><div className="component-template contract">{["Label · Editable text", "Required · Boolean, renders a red asterisk", "Size · Medium (48), Large (56)", "State · Default, Hover, Focus, Filled, Error, Disabled", "Leading icon · Boolean + instance swap, independent of trailing", "Trailing icon · Boolean + instance swap, independent of leading", "Info icon · Boolean, sits beside the label", "Hint text · Optional line below the field, switches to error color in the Error state"].map((x, i) => <div key={x}><span>{String(i + 1).padStart(2, "0")}</span>{x}</div>)}</div></section>
    </>
  );
}
